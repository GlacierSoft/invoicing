...
<script type="text/javascript">
	...
	glacier.purchase_mgr.purchaseArrival_mgr.purchaseArrival.param = {
			...
			actions : {
				batchCancelAudit:{flag:'batchCancelAudit',controlType:'multiple'},
			}
	};
	...
	glacier.purchase_mgr.purchaseArrival_mgr.purchaseArrival.purchaseArrivalDataGrid = $('#purchaseArrivalDataGrid').datagrid({
		...
		url: ctx + '/do/purchaseArrival/list.json?authstr=authstr',
	...
	//点击取消审核按钮触发方法
	glacier.purchase_mgr.purchaseArrival_mgr.purchaseArrival.batchCancelAuditPurchaseArrival = function(){
		var rows = glacier.purchase_mgr.purchaseArrival_mgr.purchaseArrival.purchaseArrivalDataGrid.datagrid("getChecked");
		var purchaseArrivalIds = [];//存储取消审核的id标识
		for(var i =0;i<rows.length;i++){
			purchaseArrivalIds.push(rows[i].purArrivalId);
		}
		if(purchaseArrivalIds.length > 0){
			$.messager.confirm('请确认', '是否要重置所选的审核信息', function(r){
				if (r){
					$.ajax({
						   type: "POST",
...
<div class="easyui-layout" data-options="fit:true">
	<div id="creditGridPanel" data-options="region:'center',border:true" >
		<table id="purchaseArrivalDataGrid">
			<glacierui:toolbar panelEnName="BatchCancelAuditArrList" toolbarId="purchaseArrivalDataGrid_toolbar" menuEnName="batchCancelAuditArr"/><!-- 自定义标签：自动根据菜单获取当前用户权限，动态注册方法 -->
		</table>
	</div>
